    ...
    const aux = await Coins.findOne({
      ...
      if (data) {//SI LA MONEDA EXISTE
        ...
        if (walletBalance < req.body.total) {
          ...
          console.log("(YA EXISTE) Se resta "+ req.body.total+" a "+ walletBalance + " y da como resultado: "+ total);
        ...
        if(walletBalance < req.body.total)
        {
          ...
          let total =  parseFloat(walletBalance) - parseFloat(req.body.total);
          console.log("(SE CREA LA COIN) Se resta "+ req.body.total+" a "+ walletBalance + " y da como resultado: "+ total);
          updateWalletBalance(req, res, total);
        }
        if (flag !== false) {
          const response = await Coins.create(modelCoin)
            .then(async (data) => {
              return {
                error: false, data: data, message: "Coin creada"};
            })
            .catch((e) => {
              flag = false;
              if (e.name == "SequelizeUniqueConstraintError" || e.name == "SequelizeValidationError") {
                res.status(409).json({ error: true, message: e.errors.map((err) => err.message)});
              } else {
        ...
        if (data.dataValues.amount < modelCoin.amount) {
          ...
          const response = await Coins.update(
            ...
            .then(async (data) => {
              ...
              let total = parseFloat(walletBalance) + parseFloat(req.body.total);
              updateWalletBalance(req,res,total);
              const res = { error: false, data: data, message: "Saldo y balance actualizado" };
              return res;
            })
            .catch((err) => {
              flag = false;
